...
#[napi(object, codegen = "inline")]
pub struct SmallInline {
  pub a: String,
  pub b: u32,
}

#[napi(object, codegen = "compact")]
pub struct SmallCompact {
  pub a: String,
  pub b: u32,
}

#[napi(object, codegen = "auto")]
pub struct SmallAuto {
  pub a: String,
  pub b: u32,
}

#[napi(object, codegen = "inline")]
pub struct MediumInline {
  pub name: String,
  pub count: u32,
  pub enabled: bool,
...
pub fn consume_medium_inline(input: MediumInline) -> u32 {
  input.count + input.tags.map(|tags| tags.len() as u32).unwrap_or_default()
}
...
pub fn consume_structured_inline(input: StructuredInline) -> u32 {
  score_structured_inline(input)
}

#[napi]
pub fn consume_structured_compact(input: StructuredCompact) -> u32 {
  score_structured_compact(input)
}

#[napi]
pub fn consume_structured_auto(input: StructuredAuto) -> u32 {
  score_structured_auto(input)
}

fn score_structured_inline(input: StructuredInline) -> u32 {
  match input {
...
fn score_structured_compact(input: StructuredCompact) -> u32 {
  match input {
...
fn score_structured_auto(input: StructuredAuto) -> u32 {
  match input {
